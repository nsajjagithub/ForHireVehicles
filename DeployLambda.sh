#!/bin/bash
#DeployLambda.sh mybucket-ns
if [[ $# -eq 0 ]] ; then
    echo 'Please enter your bucket name as ./DeployLambda.sh your-bucket'
    exit 0
fi
...
echo '[
  {
    "Id": "1",
    "Arn": "arn:aws:lambda:'$AWS_REGION':'$AWS_ID':function:ForHireVehicles"
  }
]' > ./targets.json
...
aws lambda create-function --function-name ForHireVehicles --runtime python3.7 --role  arn:aws:iam::$AWS_ID":"role/lambda-s3-role --handler lambda_function.lambda_handler --zip-file fileb://myDeploymentPackage.zip  --timeout 60 --output text >> setup.log
...
aws lambda add-permission --function-name ForHireVehicles --statement-id my-scheduled-event --action 'lambda:InvokeFunction' --principal events.amazonaws.com --source-arn arn:aws:events:$AWS_REGION:$AWS_ID:rule/my-scheduled-rule --output text >> setup.log
